...
function Products() {
    ...
    const { data, status } = useQuery(
        [PRODUCTS_QUERY_KEY, { filter: location.search }],
                    ...
                    <ul className="flex flex-wrap justify-center gap-10 mt-10 h-full">
                        {
                            data && data.length > 0 && status === 'success' && data.map((product) => (
                                <li className="flex flex-col justify-center items-center w-52 h-fit rounded-xl border-white overflow-hidden shadow shadow-slate-300 cursor-pointer"
                                    ...
                                    <p className="w-5/6 z-10 bg-white text-gray-700 capitalize text-center truncate">{product.title}</p>
                                    <p className="w-5/6 z-10 bg-white text-gray-700 capitalize text-center truncate">{product.category.name}</p>
                                    <p className="w-5/6 z-10 bg-white font-medium text-gray-600 text-center text-lg truncate">${product.price}</p>
                                    <button
