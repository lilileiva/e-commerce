        ...
        <div className="relative w-full h-fit rounded-md">
            <div className="overflow-hidden relative w-full h-max flex justify-between items-center">
                ...
                {data.images.length > 1 && <div className="w-full flex justify-between absolute">
                    <button
                        className="z-10 h-10 w-10 ml-2 bg-gray-200 opacity-60 cursor-pointer rounded-full shadow hover:bg-gray-400 duration-200 flex justify-center items-center"
                        onClick={prevImage}
                    >
                        <LeftArrowIcon size='20' />
                    </button>
                    <button
                        className="z-10 h-10 w-10 mr-2 bg-gray-200 opacity-60 cursor-pointer rounded-full shadow hover:bg-gray-400 duration-200 flex justify-center items-center"
                        onClick={nextImage}
                    >
            ...
            <ul className="flex w-full h-fit overflow-x-scroll py-2 gap-2 roudned-md">
                {data?.images.map((image, i) => (
                    <li className={`"w-32 h-32 relative rounded-md cursor-pointer ${current === i ? "shadow-lg" : ""} `}
                        key={i}
